...
from gpt_volunteer_search import necessary_equip
from main import path_to_used_organizations
...
def get_response():
    print("started")
    system = f"""
    You are helpful assistant that finds international volunteering organizations for user.
    Your task is to create a list of volunteering organizations that are not in the provided BLACKLIST       
    """
    ...
    DO NOT INCLUDE THESE ORGANIZATIONS, BLACKLIST ---------> {BLACKLIST} <--------- BLACKLIST, DO NOT INCLUDE THESE ORGANIZATIONS
...
def check_for_relevant(list_to_check: list[dict]):
    ...
    the user are not in their blacklist. Lastly, ensure that these organizations are capable of providing assistance to 
    citizens of Ukraine.
